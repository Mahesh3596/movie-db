...
const MostPopular = ({medias=[], imageBaseURL=''}) => {
    return (<div style={{width: '100%', height: '250px', overflow: 'auto', whiteSpace: 'nowrap', borderRadius: '10px', paddingBottom: '10px', display: 'flex'}}>
        ...
        {medias?.backdrops && <img loading="lazy" height="100%" src={`${imageBaseURL}/w780${medias.backdrops[0].file_path}`}/>}
        {medias?.posters && <img loading="lazy" height="100%" src={`${imageBaseURL}/w342${medias.posters[0].file_path}`}/>}
    </div>)
}
...
const MediaBackdrops = ({medias=[], imageBaseURL=''}) => {
    return (<div style={{width: '100%', height: '250px', overflow: 'auto', whiteSpace: 'nowrap', borderRadius: '10px', paddingBottom: '10px', display: 'flex'}}>
        {medias?.backdrops && medias.backdrops.map(backdrop => <img loading="lazy" key={backdrop.file_path} height="100%" src={`${imageBaseURL}/w780${backdrop.file_path}`}/>)}
    </div>)
}
const MediaPosters = ({medias=[], imageBaseURL=''}) => {
    return (<div style={{width: '100%', height: '250px', overflow: 'auto', whiteSpace: 'nowrap', borderRadius: '10px', paddingBottom: '10px', display: 'flex'}}>
        {medias?.posters && medias.posters.map(poster => <img loading="lazy" key={poster.file_path} height="100%" src={`${imageBaseURL}/w342${poster.file_path}`}/>)}
    </div>)
}
